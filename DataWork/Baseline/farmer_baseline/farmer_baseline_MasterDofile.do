...
*iefolder*1*FolderGlobals*farmer_baseline***************************************
*iefolder will not work properly if the line above is edited
   ...
   *Encrypted round sub-folder globals
   ...
   global farmer_baseline_dtRaw  "$farmer_baseline_encrypt/Raw Identified Data" 
   global farmer_baseline_doImp  "$farmer_baseline_encrypt/Dofiles Import" 
   global farmer_baseline_HFC    "$farmer_baseline_encrypt/High Frequency Checks" 

   *DataSets sub-folder globals
   global farmer_baseline_dt     "$farmer_baseline/DataSets" 
   global farmer_baseline_dtDeID "$farmer_baseline_dt/Deidentified" 
   global farmer_baseline_dtInt  "$farmer_baseline_dt/Intermediate" 
   global farmer_baseline_dtFin  "$farmer_baseline_dt/Final" 

   *Dofile sub-folder globals
   global farmer_baseline_do     "$farmer_baseline/Dofiles" 
   global farmer_baseline_doCln  "$farmer_baseline_do/Cleaning" 
   global farmer_baseline_doCon  "$farmer_baseline_do/Construct" 
   global farmer_baseline_doAnl  "$farmer_baseline_do/Analysis" 

   *Output sub-folder globals
   global farmer_baseline_out    "$farmer_baseline/Output" 
   global farmer_baseline_outRaw "$farmer_baseline_out/Raw" 
   global farmer_baseline_outFin "$farmer_baseline_out/Final" 

   *Questionnaire sub-folder globals
   global farmer_baseline_prld   "$farmer_baseline_quest/PreloadData" 
   global farmer_baseline_doc    "$farmer_baseline_quest/Questionnaire Documentation" 
...
*iefolder will not work properly if the line above is edited
   ...
   *            farmer_baseline should be replicated, including output of 
   *            tables, graphs, etc.
   ...
   * run to 1. To not run a task, set the local to 0.
   ...
   if (`importDo' == 1) { // Change the local above to run or not to run this file
       do "$farmer_baseline_doImp/farmer_baseline_import_MasterDofile.do" 
   }

   if (`cleaningDo' == 1) { // Change the local above to run or not to run this file
       do "$farmer_baseline_do/farmer_baseline_cleaning_MasterDofile.do" 
   }

   if (`constructDo' == 1) { // Change the local above to run or not to run this file
       do "$farmer_baseline_do/farmer_baseline_construct_MasterDofile.do" 
   }

   if (`analysisDo' == 1) { // Change the local above to run or not to run this file
       do "$farmer_baseline_do/farmer_baseline_analysis_MasterDofile.do" 
   }
